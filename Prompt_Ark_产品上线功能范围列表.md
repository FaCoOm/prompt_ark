...
| 状态 | 说明 | 功能数量 |
|------|------|----------|
| Confirmed | 代码实现完整，证据充分 | 21 项 |
| Partial | AI 内核存在，UI 闭环待验证 | 0 项 |
| Deprecated | 功能已禁用或废弃 | 0 项 |
...
| 状态 | 功能数量 | 功能列表 |
|------|----------|----------|
| **Confirmed** | 21 项 | Prompt 编辑、版本历史与回滚、Prompt 库浏览与筛选、Prompt 选择与注入、Prompt 运行时参数化、快捷文本处理、网页文本捕获与 Prompt 化、AI Prompt 优化、Prompt 翻译、Image-to-Prompt、Video URL → Video Prompt、Prompt 导入、Prompt 导出、外部同步后端、Prompt Hub 发布、外部社交分发、Hub 浏览与发现、社区投票、Hub 安装到扩展、Hub Fork 到个人收藏、AI Provider 配置、Hub 账号登录与扩展会话同步、应用形态与本地化 |
| **Partial** | 0 项 | — |
| **Deprecated** | 0 项 | — |
