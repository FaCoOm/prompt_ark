...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  const defaults = DEFAULT_PROMPTS.map((p, i) => {
    ...
    return {
      ...
      title: p.title,
      content: p.content,
      output_modality: p.output_modality || p.category_type || 'text',
...
async function buildSavedPromptMetadata(content, metadata = {}) {
  const locale = await getCurrentLocale();
  ...
  const categoryState = await normalizePromptForStorage({
    ...
    defaultOutputModality: metadata.output_modality || inferOutputModality(content)
  });
  ...
  return {
    ...
    output_modality: categoryState.output_modality || inferOutputModality(content),
    ...
    switch (message.type) {
      ...
      case 'IMPORT_PROMPTS': {
        ...
        for (const p of message.prompts) {
          const metadata = await buildSavedPromptMetadata(p.content, {
            title: p.title,
            category: p.category,
