...
function extractMetadataInput(p, overrides = {}) {
  return {
    title: p.title,
    category: p.category,
  ...
  try {
    switch (message.type) {
      ...
      case 'SAVE_PROMPT': {
        ...
        const metadata = await buildSavedPromptMetadata(message.prompt.content, extractMetadataInput(message.prompt), {
      ...
      case 'IMPORT_PROMPTS': {
        ...
        for (const p of message.prompts) {
          ...
          const metadata = await buildSavedPromptMetadata(preparedPrompt.content, extractMetadataInput(preparedPrompt), {
            locale: await getCurrentLocale(),
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  await handleContextMenuClick(info, tab, getPrompts, asyncEnrichPrompt, () => debouncedRebuildContextMenus());
});
...
function debouncedRebuildContextMenus() {
  if (_rebuildTimer) clearTimeout(_rebuildTimer);
  _rebuildTimer = setTimeout(() => {
    rebuildContextMenusForActiveTab().catch(err => console.error('[ContextMenu] Debounced rebuild failed:', err));
  }, 100);
}

// Build menus on install/startup
chrome.runtime.onInstalled.addListener(() => debouncedRebuildContextMenus());
chrome.runtime.onStartup.addListener(() => debouncedRebuildContextMenus());

chrome.tabs.onActivated.addListener(() => {
  debouncedRebuildContextMenus();
});
