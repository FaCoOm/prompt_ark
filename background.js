...
function parseVideoUrl(url) {
  try {
    ...
    // YouTube standard: youtube.com/watch?v= or youtu.be/
...
async function fetchVideoMetadata(platform, videoUrl) {
  switch (platform) {
    case 'youtube':
    case 'youtube-shorts':
    ...
    case 'douyin':
    case 'kuaishou':
      // No free public API — return minimal info
...
async function generateVideoPromptWithAI(videoUrl, mode = 'both', targetLang = '') {
  ...
  try {
    ...
    // Load Phase 2 system prompt based on mode
    ...
    // Inject vertical short-form instructions for 9:16 platforms
    ...
    // Mode-specific schema extensions
    ...
    if (provider.type === 'gemini') {
      ...
      // Phase 1: For YouTube, Gemini has native YouTube integration (transcripts, metadata, content)
      // file_data with YouTube URL triggers Gemini's built-in YouTube API — NOT frame-by-frame processing
      ...
      try {
        ...
        // Plain fetch — NO abort signal of any kind.
        // Chrome MV3 Service Worker corrupts both AbortController (setTimeout distortion)
        // and AbortSignal.timeout() on fetches after SW hibernation. Let HTTP handle timeout.
      ...
      if (raw) {
        try {
          ...
          // Gemini output may be truncated — attempt repair by closing open brackets
          console.warn('[Video Prompt] JSON truncated, attempting repair...', parseErr.message);
          let repaired = raw;
          // Find the last complete property by trimming to last valid closing point
      ...
      // Attach Phase 1 data to result for UI display
      ...
      // Phase 1 vocabulary (from actual video frames) overrides Phase 2's text-guessed vocabulary
      if (result && phase1Vocabulary.length > 0) result.visual_vocabulary = phase1Vocabulary;
      if (result) result._mode = mode;
      // Defense: truncate degenerate variables (Gemini sometimes generates infinite repetitive text)
      if (result?.variables) {
    ...
    if (provider.type === 'gemini-web') {
      ...
      // Phase 1: For YouTube, let Gemini Web watch and deeply analyze the video first
      if (isYouTubePlatform) {
        try {
          ...
          // Extract transcript and summary from structured output
          const extracted = parseVideoAnalysis(analysis);
          videoTranscript = extracted.transcript;
          videoSummary = extracted.summary || extracted.fullText.slice(0, 1500);

          // Build enriched content for Phase 2
          phase2Content = `<video>\n${buildVideoUserContent(parsed.videoUrl, metadata, parsed.platform, parsed.isVertical)}\n</video>\n\n<video_analysis>\n${analysis.trim()}\n</video_analysis>`;
        } catch (e) {
      ...
      // Phase 2: Generate shot prompts from the enriched content
      chrome.runtime.sendMessage({ type: 'VIDEO_PROMPT_PROGRESS', step: 2, message: '🎯 Phase 2/2: 正在基于真实内容生成分镜 Prompt...' }).catch(() => { });
...
${phase2Content}`;
      ...
      // Attach Phase 1 transcript/summary to result for UI display
