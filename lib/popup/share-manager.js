        ...
        if (platform === 'json') {
            if (prompt) {
                const json = JSON.stringify({
                    ...
                    output_modality: prompt.output_modality,
...
💬 完整prompt模板：${url}`;
    ...
    async publishToHub(id) {
        ...
        const prompt = prompts.find(p => p.id === id);
        if (!prompt) {
            showToast('❌ ' + i18n.t('sharePromptNotFound'));
            this._setSharePanelBusy(false);
            return;
        }

        showToast(i18n.t('sharePublishingToHub'));
        ...
        try {
            ...
            showToast(i18n.t('sharePublishedToHub'));

            prompt.hubSubmissionId = resp.id;
            prompt.hubSubmittedAt = new Date().toISOString();
    ...
    async publishPackToHub(ids) {
        if (ids.length === 0) { showToast(i18n.t('packSelectOne')); return; }

        const prompts = this._getPrompts();
        const selectedPrompts = ids.map(id => prompts.find(p => p.id === id)).filter(Boolean);

        showToast(i18n.t('sharePublishingPromptsToHub'));
        ...
        try {
            ...
            showToast(i18n.t('sharePublishedPromptsToHub', { count: resp.ids?.length || selectedPrompts.length }));
            this.exitPackMode();
            chrome.tabs.create({ url: resp.url });
        } catch (e) {
