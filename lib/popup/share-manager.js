...
export class ShareManager {
    ...
    _normalizeRedditBody(body, shareUrl = '') {
    ...
    async _handleNotLoggedIn(action, prompt, platform = null) {
        ...
        const resolvedPrompt = prompt
            || this._sharePrompt
            || (this._sharePromptId
                ? prompts.find(p => String(p.id) === String(this._sharePromptId))
                : null)
            || prompts.find(p => p.title === this._shareTitle)
            || null;
        ...
        const promptData = platform
            ? {
                ...(resolvedPrompt || {}),
                ...(prompt || {}),
                title: (prompt?.title || resolvedPrompt?.title || this._shareTitle || 'AI Prompt'),
                content: (prompt?.content || resolvedPrompt?.content || prompt?.prompt || resolvedPrompt?.prompt || ''),
                platform
            }
    ...
    async handleShareOption(platform) {
        ...
        // Get prompt data
        const prompts = this._getPrompts();
        const prompt = this._sharePrompt
            || (this._sharePromptId
                ? prompts.find(p => String(p.id) === String(this._sharePromptId))
                : null)
            || prompts.find(p => p.title === title);

        // Reddit: ensure share URL first, then AI-polish title/body, then open + inject
        if (platform === 'reddit') {
            const content = prompt?.content || prompt?.prompt || '';
            let redditTitle = title || 'AI Prompt';
            let redditBody = content;
            let shareUrl = url;
            ...
            // AI polish (same as original flow)
            if (content.length > 0) {
                ...
                try {
                    ...
                    if (aiResult?.success) {
                        redditTitle = aiResult.title || redditTitle;
                        redditBody = aiResult.body || redditBody;
                    }
                } catch (e) {
                    console.warn('[ShareManager] Reddit AI polish failed, using raw content:', e);
                }
                this._dismissShareToast();
            }
            redditBody = this._normalizeRedditBody(redditBody, effectiveUrl);

            // Fire-and-forget — no awaits after this point
            navigator.clipboard.writeText(redditBody || redditTitle).catch(() => {});
            chrome.runtime.sendMessage({
                type: 'OPEN_REDDIT_PREFILL',
                title: redditTitle,
                text: redditBody,
            }).catch(() => {});
            this._setSharePanelBusy(false);
            this._showShareToast(i18n.t('shareRedditCopied'), 4500);
            this.hideSharePanel();
            return;
        }
        ...
        if (socialPlatforms.includes(platform)) {
            ...
            // --- URL-based platforms (Twitter) + clipboard (WeChat) ---
            let aiResult = null;
...
💬 完整prompt模板：${url}`;
    ...
    enterPackMode() {
        ...
        document.getElementById('packBtn')?.classList.add('active');
    ...
    exitPackMode() {
        ...
        document.getElementById('packBtn')?.classList.remove('active');
    }
