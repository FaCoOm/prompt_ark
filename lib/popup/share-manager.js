...
export class ShareManager {
    ...
    _buildPendingIntentSummary(action, promptData) {
        const title = promptData?.packTitle || promptData?.title || 'Untitled';
        ...
        if (action === 'PUBLISH_PACK_TO_HUB') {
            const key = promptData?.visibility === 'public'
                ? 'pendingActionPublishPack'
                : 'pendingActionSharePack';
...
💬 完整prompt模板：${url}`;
    ...
    async publishPackToHub(ids, packTitle) {
        ...
        try {
            ...
            if (!resp.success) {
                if (resp.error === 'NOT_LOGGED_IN') {
                    await this._handleNotLoggedIn('PUBLISH_PACK_TO_HUB', { prompts: packPrompts, packTitle, visibility: 'public' });
                } else {
                    showToast('❌ ' + resp.error);
                }
                return;
    ...
    async sharePack() {
        ...
        try {
            ...
            if (!resp.success) {
                if (resp.error === 'NOT_LOGGED_IN') {
                    await this._handleNotLoggedIn('PUBLISH_PACK_TO_HUB', { prompts: packPrompts, packTitle, visibility: 'unlisted' });
                } else {
                    showToast('❌ ' + resp.error);
                }
                return;
