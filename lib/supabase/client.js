...
function buildPromptPayload(prompt, visibility, authorMeta, matches = {}, options = {}) {
  ...
  return {
    ...
    source_metadata: {
      ...
      hub_prompt_id: prompt?.hub_prompt_id || null,
      origin_action: prompt?.origin_action || null,
...
async function buildHubPromptUrl() {
  return await getHubUrl();
}
...
export const HubClient = {
  ...
  async publishPrompt(prompt, visibility = 'unlisted') {
    ...
    return {
      ...
      url: isPublicHubPublish
        ? await buildHubPromptUrl()
        : await buildHubSubmissionUrl(data[0]?.id),
      visibility,
  ...
  async publishPrompts(prompts, visibility = 'public') {
    ...
    return {
      ...
      url: visibility === 'public'
        ? await buildHubPromptUrl()
        : (ids.length === 1 ? await buildHubSubmissionUrl(ids[0]) : await buildHubPromptUrl()),
      visibility,
      reviewStatus: 'pending',
      submissionType: 'hub_publish',
    };
