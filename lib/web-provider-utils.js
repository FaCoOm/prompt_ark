...
export async function getCookieHeader(domain, url) {
    try {
        ...
        console.log(`[WebProviderUtils] Fetching cookies for domain=${domain}, url=${url}...`);
        
        const allCookies = [];
        
        try {
            const urlCookies = await chrome.cookies.getAll({ url });
            console.log(`[WebProviderUtils] Method 1 (url=${url}): ${urlCookies.length} cookies`);
            allCookies.push(...urlCookies);
        } catch (e) {
            console.log('[WebProviderUtils] Method 1 failed:', e.message);
        }
        
        if (domain) {
            try {
                const domainCookies = await chrome.cookies.getAll({ domain });
                console.log(`[WebProviderUtils] Method 2 (domain=${domain}): ${domainCookies.length} cookies`);
                allCookies.push(...domainCookies);
            } catch (e) {
                console.log('[WebProviderUtils] Method 2 failed:', e.message);
            }
        }
        ...
        if (domain?.startsWith('.')) {
            ...
            try {
                const noDotCookies = await chrome.cookies.getAll({ domain: domainWithoutDot });
                console.log(`[WebProviderUtils] Method 3 (domain=${domainWithoutDot}): ${noDotCookies.length} cookies`);
                allCookies.push(...noDotCookies);
            } catch (e) {
                console.log('[WebProviderUtils] Method 3 failed:', e.message);
            }
        }
        ...
        try {
            ...
            const filtered = all.filter(c => 
                c.domain.includes('xiaomimimo') || 
                c.domain.includes('xiaomi')
            );
            console.log(`[WebProviderUtils] Method 4 (all cookies filter): ${filtered.length} of ${all.length} cookies match xiaomimimo/xiaomi`);
            console.log('[WebProviderUtils] Filtered cookies:', filtered.map(c => ({name: c.name, domain: c.domain})));
            console.log('[WebProviderUtils] All cookie domains:', [...new Set(all.map(c => c.domain))].slice(0, 20));
            allCookies.push(...filtered);
        } catch (e) {
            console.log('[WebProviderUtils] Method 4 failed:', e.message);
        }
        ...
        if (!uniqueCookies.length) {
            console.log('[WebProviderUtils] No cookies found with any method');
            return '';
        }
        
        const cookieString = uniqueCookies.map(c => `${c.name}=${c.value}`).join('; ');
        console.log('[WebProviderUtils] Final cookies:', uniqueCookies.map(c => c.name));
        return cookieString;
    } catch (e) {
        console.error(`[WebProviderUtils] Error getting cookies:`, e);
        return '';
    }
}
