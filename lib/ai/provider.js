...
export async function callCloudAPI(text, lang) {
                ...
                body: JSON.stringify({
                    ...
                    generationConfig: {
                        ...
                        responseSchema: {
                            ...
                            required: ['title', 'output_modality', 'recommended_category_key', 'confidence'],
                        },
                    },
                }),
            }
    ...
    if (provider.type === 'openai') {
        const resp = await fetchWithTimeout(`${provider.apiUrl}/chat/completions`, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    { role: 'system', content: systemPrompt + ' Return JSON only: {"title":"...","output_modality":"text","recommended_category_key":"general_productivity","confidence":0.88,"tags":["...","..."]}' },
    ...
    if (provider.type === 'gemini-web') {
        const webPrompt = `${systemPrompt} Return JSON only: {"title":"...","output_modality":"text","recommended_category_key":"general_productivity","confidence":0.88,"tags":["...","..."]}
