        ...
        if (tabId === newTab.id && changeInfo.status === 'complete') {
            if (platform === 'wechat') {
                ...
                if (wechatState.redirectUrl) {
                    try {
                        ...
                        cleanup();
                        safeSendResponse({ success: false, error: 'WeChat editor redirect failed' });
                    }
                    return;
                }
            ...
            setTimeout(async () => {
                try {
                    ...
                    safeSendResponse({ success: true, text: shareText, injected: true });
                } catch (e) {
                    ...
                    try {
                        ...
                        safeSendResponse({ success: false, error: 'Injection failed' });
                    }
                }
            }, 3000);
        }
