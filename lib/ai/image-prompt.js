...
export async function generateImagePrompt(imageUrl, imageModelId = null) {
  ...
  try {
    ...
    if (provider.type === 'gemini') {
      result = await generateWithGeminiAPI(imageUrl, systemPrompt, provider, targetLanguage);
    } else if (provider.type === 'gemini-web') {
      throw new Error('Image-to-prompt is not supported with Gemini Web. Please use an OpenAI-compatible provider.');
    } else if (provider.type === 'openai') {
      result = await generateWithOpenAI(imageUrl, systemPrompt, provider, targetLanguage);
    } else {
...
async function generateWithGeminiAPI(imageUrl, systemPrompt, provider, targetLanguage = 'Chinese') {
  ...
  const userPrompt = `Analyze this image and generate a structured prompt following the output format. IMPORTANT: All descriptions must be in ${targetLanguage}.`;
...
async function generateWithOpenAI(imageUrl, systemPrompt, provider, targetLanguage = 'Chinese') {
  ...
  // Build user prompt with language instruction
  const userPrompt = `Analyze this image and generate a structured prompt. IMPORTANT: All descriptions must be in ${targetLanguage}.`;
