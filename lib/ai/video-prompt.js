...
${phase2Content}`;
        ...
        if (provider.type === 'kimi-web') {
            // Kimi Web doesn't support native video analysis like Gemini,
            // so we use metadata-only approach similar to OpenAI
            chrome.runtime.sendMessage({ type: 'VIDEO_PROMPT_PROGRESS', step: 1, message: '📄 使用视频元数据进行生成 (Kimi Web 不支持视频分析)...' }).catch(() => { });
            console.log('[Video Prompt] Using metadata-only approach for Kimi Web');

            chrome.runtime.sendMessage({ type: 'VIDEO_PROMPT_PROGRESS', step: 2, message: '🎯 正在基于元数据生成分镜 Prompt...' }).catch(() => { });
            const webPrompt = `${systemPrompt}
...
${wrappedContent}`;
            ...
            console.log('[Video Prompt] Generation complete:', result.length, 'chars');
            result = result.replace(/^```(?:json)?\s*/i, '').replace(/\s*```\s*$/i, '').trim();
            const jsonStart = result.indexOf('{');
            const jsonEnd = result.lastIndexOf('}');
        ...
        if (provider.type === 'deepseek-web') {
            // DeepSeek Web doesn't support native video analysis,
            // so we use metadata-only approach
            chrome.runtime.sendMessage({ type: 'VIDEO_PROMPT_PROGRESS', step: 1, message: '📄 使用视频元数据进行生成 (DeepSeek Web 不支持视频分析)...' }).catch(() => { });
            console.log('[Video Prompt] Using metadata-only approach for DeepSeek Web');
