...
Return JSON only: {"prompt":"...","title":"...","category":"...","tags":[...]}`;
    ...
    if (provider.type === 'deepseek-web') {
        const webPrompt = `TASK: Convert the selected text into a structured prompt.
...
{"prompt":"...","title":"...","category":"...","tags":["..."]}`;
        ...
        console.log('[smart-convert] DeepSeek raw result:', result.substring(0, 200));
        
        result = result.trim();
        
        // DeepSeek Web sometimes returns content without the opening brace
        // Manually add it if missing
        if (!result.startsWith('{') && !result.startsWith('"{')) {
            result = '{' + result;
            console.log('[smart-convert] Added missing opening brace');
        }
        
        // Also ensure it ends with }
        if (!result.endsWith('}')) {
            ...
            if (lastBrace > 0) {
                ...
                result = result + '}';
                console.log('[smart-convert] Added missing closing brace');
            }
        }
        
        console.log('[smart-convert] Final result to parse:', result.substring(0, 200));
        
        try {
            const parsed = JSON.parse(result);
            console.log('[smart-convert] Successfully parsed JSON');
            return parsed;
        } catch (e) {
            console.error('[smart-convert] JSON parse error:', e.message);
            return null;
        }
    }
