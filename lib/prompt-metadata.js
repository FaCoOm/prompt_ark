...
async function deriveInitialCategoryState(content, metadata = {}, options = {}) {
  ...
  if (manualCustomCategory) {
    ...
    if (manualKey) {
      return {
        category_type: reviewConfirmed ? CATEGORY_TYPES.CUSTOM : CATEGORY_TYPES.PENDING,
  ...
  if (categorySource === 'mine' && explicitType === CATEGORY_TYPES.CUSTOM && explicitKey) {
    return {
  ...
  if (categorySource === 'custom' && explicitType === CATEGORY_TYPES.CUSTOM && explicitKey) {
    return {
      category_type: reviewConfirmed ? CATEGORY_TYPES.CUSTOM : CATEGORY_TYPES.PENDING,
      category_key: explicitKey,
      classification_confidence: normalizeConfidence(metadata.classification_confidence, 1),
