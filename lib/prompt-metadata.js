...
function hasManualCustomCategory(metadata = {}) {
  return metadata.category_source === 'custom'
    && metadata.category_type === CATEGORY_TYPES.CUSTOM
    && normalizeText(metadata.category_key);
}
...
function deriveInitialOutputModality(content, metadata = {}, options = {}) {
  ...
  if (explicitOutputModality === 'image') {
    return {
...
async function deriveInitialCategoryState(content, metadata = {}, options = {}) {
  ...
  const manualCustom = hasManualCustomCategory(metadata);
  ...
  if (existingPrompt && !categoryChanged) {
    return {
  ...
  if (manualCustom) {
    return {
      ...
      category_key: normalizeText(metadata.category_key),
      classification_confidence: normalizeConfidence(metadata.classification_confidence),
      needs_category_review: false,
    };
  }
  ...
  if (explicitType === CATEGORY_TYPES.CUSTOM && explicitKey) {
    return {
