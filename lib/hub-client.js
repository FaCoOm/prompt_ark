...
export const HubClient = {
    ...
    async publishPrompt(prompt, token) {
        ...
        // Step 3: Create public Listing Gist
        ...
        return {
            gistId,
            hubUrl: `https://gist.github.com/${gistId}`,
        };
    },
    ...
    async _fetchAuthorInfo(token) {
        try {
            ...
            if (resp.ok) {
                ...
                return { author: user.login, authorAvatar: user.avatar_url };
            }
        } catch (e) {
    ...
    async _registerInIndex(token, entry) {
        ...
        try {
            ...
            console.error('[HubClient] Failed to register in index:', e);
            // Non-fatal: listing Gist was still created successfully
        }
    },
