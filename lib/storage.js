...
export const PromptStorage = {
    ...
    async get() {
        ...
        if (SyncManager.backend === 'none' || SyncManager.backend === 'webdav' || SyncManager.backend === 'obsidian') {
            ...
            const rawPrompts = await LocalStorage.get('prompts') || [];
            const { prompts, changed } = await normalizePromptsForStorage(rawPrompts);
            if (changed) {
    ...
    async getById(id) {
        ...
        const prompt = locals.find(p => p.id === id) || null;
        return prompt ? await normalizePromptForStorage(prompt) : null;
    },
