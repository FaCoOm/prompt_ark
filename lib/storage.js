...
async function normalizePromptReadResult(prompts = [], options = {}) {
    const normalizedBase = (prompts || []).map(normalizePromptUsage);
    const { prompts: normalizedPrompts, changed } = await normalizePromptsForStorage(normalizedBase);

    if (changed && options.persistLocal) {
...
export const PromptStorage = {
    ...
    async get() {
        ...
        if (SyncManager.backend === 'none' || SyncManager.backend === 'webdav' || SyncManager.backend === 'obsidian') {
            ...
            return await normalizePromptReadResult(prompts, { persistLocal: true });
        }
        ...
        if (ids && Array.isArray(ids)) {
            ...
            return await normalizePromptReadResult(prompts);
        }
        ...
        return await normalizePromptReadResult(mergePrompts(syncPrompts, localPrompts));
    },
