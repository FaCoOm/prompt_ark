...
async function _callQwenCNWebInner(prompt, model, _retrying) {
    ...
    // Read response as text (like zero-token does)
    const responseText = await response.text();
    
    console.log('[QwenCNWeb] Response length:', responseText.length);
    console.log('[QwenCNWeb] Response preview:', responseText.substring(0, 500));
    
    // Parse SSE format and extract the final content
    ...
    for (const line of lines) {
        ...
        if (trimmed.startsWith('data:')) {
            ...
            try {
                ...
                // Look for multi_load/iframe or signal/post messages
                if (data.data?.messages && Array.isArray(data.data.messages)) {
                    for (const msg of data.data.messages) {
                        if ((msg.mime_type === 'multi_load/iframe' || msg.mime_type === 'signal/post') && msg.content) {
                            finalContent = msg.content; // Use last valid content
                        }
                    }
                }
            } catch (e) {
    ...
    console.log('[QwenCNWeb] Extracted content length:', finalContent.length);
